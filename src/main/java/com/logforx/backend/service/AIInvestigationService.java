...
public class AIInvestigationService {
    ...
    public Map<String, Object> getInvestigationCopilotData() {
        ...
        // Null-safe threat type checks
        boolean hasBruteForce = detections.stream().anyMatch(d ->
                d.getThreatType() != null && d.getThreatType().toLowerCase().contains("brute"));
        boolean hasMalware = detections.stream().anyMatch(d ->
                d.getThreatType() != null && d.getThreatType().toLowerCase().contains("malicious"));
        boolean hasC2 = detections.stream().anyMatch(d ->
                d.getThreatType() != null && d.getThreatType().toLowerCase().contains("control"));
        boolean hasExfil = detections.stream().anyMatch(d ->
                d.getThreatType() != null && d.getThreatType().toLowerCase().contains("exfiltration"));

        StringBuilder summary = new StringBuilder("AI Agent detected an active multi-stage attack lifecycle: ");
        List<String> tactics = detections.stream()
